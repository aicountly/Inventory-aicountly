...
import { FileDown, Layers, ListPlus, PackageMinus, Plus, Trash2 } from 'lucide-react'
...
export function ComponentLinesCard({
  ...
  const focusQty = useCallback((key: string) => {
    requestAnimationFrame(() => {
      bodyRef.current?.querySelector<HTMLInputElement>(`[data-qty-for="${key}"]`)?.focus()
    })
  }, [])
  ...
  const onBodyKeyDown = (e: KeyboardEvent<HTMLTableSectionElement>) => {
    ...
    if (!body) return
    // DOM order IS entry order: the typeahead, then batch, warehouse, quantity, unit of each row.
    const fields = [...body.querySelectorAll<HTMLElement>('[data-line-nav], input[role="combobox"]')].filter(
  ...
  return (
    <section className={cx(AIC, 'flex min-w-0 flex-col rounded-xl border border-gray-200 bg-white shadow-card')}>
      <header className="flex flex-wrap items-center justify-between gap-2 border-b border-gray-100 px-4 py-3">
        <div className="flex min-w-0 items-center gap-2.5">
          ...
          <h2 className="truncate text-sm font-semibold text-gray-900">Components (materials out)</h2>
              ...
              return (
                ...
                >
                  ...
                  <td className={BODY_CELL}>
                    ...
                    {rowErrors.item_id ? <p className="mt-1 text-[11px] text-red-600">{rowErrors.item_id}</p> : null}
                  </td>
                  ...
                  <td className={BODY_CELL}>
                    ...
                    {line.item_id && line.track_serial ? (
                      <div className={line.track_batch ? 'mt-1' : undefined}>
                        <SerialPicker
                          variant="field"
                          itemId={line.item_id}
                          itemName={line.item_name}
                    ...
                    {line.item_id && !line.track_batch && !line.track_serial ? (
                      <span className="inline-block pt-1.5 text-xs text-gray-400" aria-label="Not tracked">
                        Not tracked
                      </span>
                    ) : null}
                    {!line.item_id ? <span className="inline-block pt-1.5 text-xs text-gray-300">—</span> : null}
                    {rowErrors.batch_id ? <p className="mt-1 text-[11px] text-red-600">{rowErrors.batch_id}</p> : null}
                    {rowErrors.serials ? <p className="mt-1 text-[11px] text-red-600">{rowErrors.serials}</p> : null}
                  </td>
                  ...
                  <td className={BODY_CELL}>
                    ...
                    />
                    {rowErrors.qty ? <p className="mt-1 text-[11px] text-red-600">{rowErrors.qty}</p> : null}
                  </td>
                  ...
                  <td className={BODY_CELL}>
                    {line.units.length > 0 ? (
                      ...
                      <span className="inline-block pt-1.5 text-xs text-gray-300">—</span>
                    )}
                  </td>
                    ...
                    <>
                      <td className={cx(BODY_CELL, 'pt-2.5 text-right text-sm tabular-nums text-gray-700')}>
                        {rowCost?.enteredUnitCost != null ? (
                          `${currencySymbol} ${formatMoney(rowCost.enteredUnitCost)}`
                        ) : line.item_id ? (
                      ...
                      <td className={cx(BODY_CELL, 'pt-2.5 text-right text-sm font-semibold tabular-nums text-gray-900')}>
                        {rowCost?.amount != null ? `${currencySymbol} ${formatMoney(rowCost.amount)}` : <span className="text-gray-300">—</span>}
                      </td>
                    </>
                  ) : null}
          ...
          {!costHidden && cost.componentCost > 0 ? (
            <>
              ...
              <strong className="font-semibold tabular-nums text-gray-800">
                {currencySymbol} {formatMoney(cost.componentCost)}
              </strong>
            </>
          ) : null}
        </span>
