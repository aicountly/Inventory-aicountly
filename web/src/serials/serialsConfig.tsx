...
/**
 * The declarative description of a serial number.
 *
 * It outlived the generic `MasterPage` screen it was written for, and on
 * purpose. Two things still read it, and both would drift if it were inlined
 * into the workspace:
 *
 *  - `columns` is the EXPORT contract. CSV, Excel, PDF and the letterheaded
 *    print sheet are all produced from this list through `masterExportColumns`,
 *    so a column's exported value is defined next to the column itself and a new
 *    one cannot be added to the sheet and forgotten — or the other way round.
 *  - `fields` is the FORM. `SerialFormDrawer` renders it with the same
 *    `MasterForm` every other master uses, which is why the item picker, the
 *    batch list that reloads when the item changes, the location list that
 *    reloads when the warehouse changes and the server-error-to-field mapping
 *    all work here without being written twice.
 */
...
export const serialsConfig: MasterConfig<Serial> = {
  ...
  fields: [
    ...
    { name: 'serial_no', label: 'Serial number', type: 'text', required: true, maxLength: 128, help: 'Letters, digits, hyphens and slashes are all kept as typed — nothing is normalised away.' },
...
/**
 * The form's fields for a reader who may not see cost.
 *
 * Dropping the field rather than disabling it: a disabled input still carries
 * the value it was given, and this form is opened on an existing record.
 */
export function serialFormFields(costVisible: boolean) {
  return costVisible ? serialsConfig.fields : serialsConfig.fields.filter((f) => f.name !== 'unit_cost')
}

/** The export columns, minus cost when the reader is not allowed to see it. */
export function serialExportColumns(costVisible: boolean) {
  return costVisible ? serialsConfig.columns : serialsConfig.columns.filter((c) => c.key !== 'unit_cost')
}
