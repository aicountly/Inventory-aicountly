...
export function AssemblyPage({ spec, documentId, initial, statusBadge, onSaved }: AssemblyPageProps) {
  ...
  // The company's own rules, read live rather than assumed: base currency and valuation method.
  ...
  // The key `ValuationController` itself authorises on, so a profile without it cannot obtain a
  // cost by any route. The columns are then absent rather than dashed, and nothing is requested.
  const costHidden = !can(P.report('valuation'))
  ...
  const costOf = useCallback((itemId: number) => unitCosts.costs.get(itemId) ?? null, [unitCosts.costs])
  const cost = useMemo(
    () => assemblyCost(split.components, split.finished, costOf),
    [split.components, split.finished, costOf],
  )

  /**
   * An amount written the way this company writes it.
   *
   * One helper passed down rather than a currency symbol handed to five components: the symbol is
   * null until the settings read lands, and null concatenated into a template leaves a stray space
   * in front of every figure on the screen.
   */
  const formatAmount = useCallback(
    (value: number) => {
      const symbol = policy.currencyCode ? currencySymbol(policy.currencyCode) : null
      return symbol ? `${symbol} ${formatMoney(value)}` : formatMoney(value)
    },
    [policy.currencyCode],
  )
  ...
  useEffect(() => {
    ...
    const controller = new AbortController()
    // Filtered by the server on the item itself — a name search would match a BOM for a different
    // item that happens to be called something similar.
    lookupApi
      .boms('', { finishedItemId, signal: controller.signal })
      .then((res) => {
        if (controller.signal.aborted) return
    ...
    <PageShell paddingBottom>
      ...
      {initialLoading ? (
        ...
        <>
          ...
          {unitCosts.error && !costHidden ? (
        <Notice kind="warning">Inventory cost could not be read, so the expected cost is not shown.</Notice>
      ) : null}
