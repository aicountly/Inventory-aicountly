  ...
  return (
    <section className={cx(AIC, 'flex min-w-0 flex-col rounded-xl border border-gray-200 bg-white shadow-card')}>
      <header className="flex flex-wrap items-center gap-2.5 border-b border-gray-100 px-4 py-3">
        ...
        <h2 className="min-w-0 flex-1 truncate text-sm font-semibold text-gray-900">Assembled item (finished good)</h2>
        ...
        {finished.item_id === null ? (
          <FormField label="Item" htmlFor={`${ids}-item`} required error={errors.finished.item_id}>
            <LineItemPicker
              variant="field"
              scan
              placeholder="Search finished item by name, SKU or barcode…"
              itemId={null}
              itemName=""
              itemSku={null}
          ...
          <div>
            ...
            <div className="flex items-center gap-3 rounded-xl border border-gray-200 bg-gray-50/60 p-2.5">
              {/* No item images exist in this product, so the tile is an icon and says so to a
                  screen reader rather than reserving space for a photograph that never arrives. */}
              <span className="flex h-12 w-12 shrink-0 items-center justify-center rounded-lg bg-white text-gray-400 ring-1 ring-gray-200" aria-hidden>
          ...
          <div className="flex items-center justify-between gap-3 rounded-xl border border-emerald-200 bg-emerald-50/50 px-3.5 py-3">
            ...
            <p className="shrink-0 text-lg font-bold tabular-nums text-emerald-700">
              {costLoading ? (
                ...
                `${currencySymbol} ${formatMoney(cost.expectedCost)}`
              )}
            </p>
          </div>
        ) : (
