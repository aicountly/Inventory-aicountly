...
/**
 * Each half of the document is a named region, so a row's item field is found inside the side it
 * belongs to. Both sides draw the SHARED typeahead, which has one placeholder — scoping by region
 * is what keeps "the component's item field" and "the assembled item's field" apart.
 */
function componentSearch(row = 0): HTMLElement {
  const region = screen.getByRole('region', { name: /components \(materials out\)/i })
  return within(region).getAllByPlaceholderText(/search item by name, sku or barcode/i)[row]
}

function finishedSearch(): HTMLElement {
  const region = screen.getByRole('region', { name: /assembled item \(finished good\)/i })
  return within(region).getByPlaceholderText(/search item by name, sku or barcode/i)
}
