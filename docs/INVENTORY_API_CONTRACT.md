...
`GET /v1/pending-quantities?kind=challan|deferred_purchase|job_work&direction&party_ref&status` (open/partial rows with `document_id`, `item_id`, `qty_original`, `qty_settled`). Each row also carries what the document that opened it says — `document_no`, `document_date`, `expected_return_date`, `party_name`, `item_sku`, `warehouse_name` — beside the register's own derived `due_date`, `ageing_days`, `days_overdue`, `status`, `priority`, `unit_cost` and `pending_value`. A pending quantity has none of the former of its own, and a caller showing "what is late and who is holding it" would otherwise read a document per row. Deliberately not scoped by financial year: goods sent to a job worker in February are still out in April.
`GET /v1/pending-quantities/{id}` — one row in any status with its settlement trail.
`GET /v1/packing-lists`, `GET /{id}`, `POST /{id}/unpack | lock | unlock`.
`GET/POST /v1/reservations`, `POST /{id}/release`, `POST /{id}/fulfil`.

## Job work
A job-work dispatch and receipt are ordinary documents (`JOB_WORK_OUT` / `JOB_WORK_IN` through `/v1/inventory-documents`) and the open position is the pending register above. These two reads are the aggregate neither of those answers; there is no separate job-work create or post path, and no second copy of the settlement rules.

`GET /v1/job-work/summary` (permission `documents.read`) → `{as_on, window{from,to}, previous_window{from,to}, open{qty,orders,items,workers}, due{…}, overdue{…}, received{qty,value,documents,lines}, received_previous{…}, sent{…}, sent_previous{…}, turnaround{days,samples}, turnaround_previous{…}}`. Company, year and branch come from the session context. `open` is every quantity still with a job worker and is NOT year-scoped; `due` and `overdue` are the parts of it whose dispatch promised a return date that has arrived or passed (a dispatch that promised none is in neither). `received.value` is the receipt's inventory value (`valuation_amount`); `sent.value` is the commercial value on the dispatch challan (`source_transaction_amount`) — different figures, never mixed. `turnaround.days` is the mean of `settling document date − dispatch date` over the settlements in the window, `null` when nothing settled; `samples` says how many it is the mean of. The period windows are the month to date, clipped to the selected financial year, and the previous one runs to the same day of the previous month so a delta compares like with like.

`GET /v1/job-work/workers?q=&limit=` (permission `documents.read`) → rows of `{party_ref, party_name, documents, last_sent_on, last_received_on, last_document_date, open_qty, open_orders}`, workers still holding material first. Read from Inventory's OWN job-work documents — the `party_ref` / `party_name` snapshot each one already carries. The ledger master is Books' and nothing here copies, mirrors or caches it; a client that needs the ledger itself opens it in Books.
