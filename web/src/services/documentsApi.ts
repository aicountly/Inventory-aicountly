...
export const documentsApi = {
  ...
  /**
   * Lines of several documents in ONE call (`GET /v1/inventory-documents/lines?document_ids=`).
   *
   ...
   */
  async linesFor(documentIds: number[], signal?: AbortSignal): Promise<Record<number, DocumentLine[]>> {
    const ids = [...new Set(documentIds.filter((id) => Number.isFinite(id) && id > 0))]
    if (ids.length === 0) return {}
    const res = await api.get<ItemResponse<Record<string, DocumentLine[]>>>(`${BASE}/lines`, {
      query: { document_ids: ids.join(',') },
      signal,
    })
    const out: Record<number, DocumentLine[]> = {}
    for (const [key, lines] of Object.entries(res.data ?? {})) {
      out[Number(key)] = Array.isArray(lines) ? lines : []
    }
    return out
  },
