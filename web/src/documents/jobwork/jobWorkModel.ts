...
export interface PendingView {
  ...
  /** What the dispatch actually promised, or null when it promised nothing. */
  expectedReturnDate: string | null
  /** The date lateness is measured against: the promise, or the company grace period. */
  dueDate: string | null
  /** False when `dueDate` is the grace period standing in for a promise. */
...
/**
 * The register derives ageing, the due date and lateness in SQL, because the
 * due date is a policy — a dispatch that promised no return date is measured
 * against the company's grace period, which the browser does not know. Those
 * fields are used where the payload carries them, so this drawer and the
 * pending register can never call the same row two different things; the local
 * arithmetic below is only the fallback for a payload without them.
 */
export function pendingView(row: PendingRow, today: string): PendingView {
  ...
  const dueDate = row.due_date ?? expected
  const dueInDays = daysBetween(today, dueDate)
  const overdue = typeof row.is_overdue === 'boolean' ? row.is_overdue : dueInDays !== null && dueInDays < 0
  ...
  return {
    ...
    dueDate,
    hasExpectedDate: typeof row.has_expected_date === 'boolean' ? row.has_expected_date : expected !== null,
    itemId: row.item_id,
    ...
    received,
    open,
    ageDays: toNumber(row.ageing_days) ?? daysBetween(row.document_date, today),
    dueInDays,
    state,
  ...
  for (const view of views) {
    ...
    if (!group) {
      group = {
        ...
        hasExpectedDate: view.hasExpectedDate,
