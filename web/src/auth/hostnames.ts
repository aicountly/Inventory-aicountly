 ...
 * inventory.aicountly.com and inventory.gh.aicountly.com. This is the fallback for
 * hosts the pattern does not cover — localhost above all.
 */
export const PRODUCT_KEY = (import.meta.env.VITE_PRODUCT_KEY ?? 'inventory').trim() || 'inventory'
...
/**
 * `inventory.aicountly.com` and `inventory.gh.aicountly.com` both resolve to
 * `inventory`, which is what the portal expects in authentication_jump.
 */
