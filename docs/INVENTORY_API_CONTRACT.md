...
`/v1/item-groups` and `/v1/stock-categories` also return `item_count` on every row (one grouped query per page, never a count per row).
`GET /v1/stock-categories/summary` → `{total, active, inactive, created_this_month, most_used: {stock_cat_id, cat_name, item_count} | null, uncategorised_items}` — company-wide aggregates the list's own pages cannot give. `most_used` is `null` while nothing is categorised; the caller says so rather than naming a category.
`POST /v1/stock-categories/bulk-status {ids:[…], is_active:0|1}` → `{updated, is_active}`. Only rows whose status actually moves are touched, and each one writes its own `stock_category.activate` / `.deactivate` audit entry with before / after.
`GET /v1/stock-categories?sort=item_count&order=desc` ranks by the same count the rows carry, over the whole company rather than the page.
