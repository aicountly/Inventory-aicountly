 ...
 */
export function RecentAssemblies({ rows, loading, error, onReload, costHidden, formatAmount }: RecentAssembliesProps) {
  ...
  const columns = useMemo<SmartColumn<RecentAssemblyRow>[]>(() => {
    ...
    if (!costHidden) {
      base.push({
        ...
        amount: true,
        accessor: (row) =>
          row.valuationTotal > 0 ? formatAmount(row.valuationTotal) : <span className="text-gray-400">—</span>,
      })
    }
    base.push({
      key: 'actions',
