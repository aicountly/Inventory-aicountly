...
`GET /v1/pending-quantities?kind=challan|deferred_purchase|job_work&direction&party_ref&status` (open/partial rows with `document_id`, `item_id`, `qty_original`, `qty_settled`). Each row also carries the document that opened it — `document_no`, `document_date`, `expected_return_date`, `party_name`, `item_sku`, `warehouse_name` — because a pending quantity has none of those of its own and a caller showing "what is late and who is holding it" would otherwise read a document per row. Deliberately not scoped by financial year: goods sent to a job worker in February are still out in April.
