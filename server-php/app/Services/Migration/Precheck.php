...
class Precheck
{
    ...
    private function consistency(BaseConnection $b, array $companies, array $present): array
    {
        ...
        $checks = [
            ...
            // A zero cost_rate is as absent as a NULL one and has to be counted with it: a replay
            // carries whichever the migration wrote as the line's cost for good.
            'posted_lines_without_cost' => "SELECT h.cmp_id, h.vch_type_id, COUNT(*) AS n, SUM(l.qty) AS qty FROM books_voucher_inventory_lines l JOIN books_voucher_headers h ON h.vch_txn_id = l.vch_txn_id WHERE h.status = 'posted' AND h.deleted_at IS NULL {$cmpIn} AND (l.cost_rate IS NULL OR l.cost_rate = 0) AND NOT (h.vch_type_id IN (23,24) AND COALESCE(h.stock_effect,'challan_only') = 'challan_only') AND NOT (h.vch_type_id = 11 AND h.stock_effect = 'defer_inward') AND NOT (h.vch_type_id IN (11,18,2,3) AND h.stock_effect = 'from_challan') AND h.vch_type_id IN (2,3,6,10,11,14,15,18,20,23,24) GROUP BY h.cmp_id, h.vch_type_id ORDER BY 1,2",
        ];
        $out = [];
