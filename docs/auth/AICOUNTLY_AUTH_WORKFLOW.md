# AICOUNTLY auth workflow (Inventory)

How Inventory signs a user in. This is the shared AICOUNTLY SaaS flow — the same
...
1. User opens `inventory.aicountly.com` (or `inventory.gh.aicountly.com`).
2. No `auth_token` → redirect to
   `{portal}/login/authentication_jump/inventory?returnUrl={origin}/auth/callback`.
...
| Inventory host | Login redirect | Auth API | Product API |
|---|---|---|---|
| `inventory.aicountly.com` | `my.aicountly.com` | `my.aicountly.com` | `inventory.aicountly.com/api` |
| `inventory.gh.aicountly.com` | `sandbox.aicountly.com` | `my.aicountly.com` | `inventory.gh.aicountly.com/api` |
...
`inventory` out of either hostname, and `isSandboxHost()` picks the portal.
...
curl https://inventory.gh.aicountly.com/api/health
...
curl -i -X POST https://inventory.gh.aicountly.com/api/global/seskey

# 3. Unrelayed paths are refused
curl -i -X POST https://inventory.gh.aicountly.com/api/global/login   # expect 404
```
