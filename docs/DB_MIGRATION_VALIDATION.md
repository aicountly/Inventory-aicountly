...
| `books_snapshot` | `books:export-inventory-snapshot` JSON (Books' StockBalanceService closing quantities and StockValuationService values, per company/FY, as-of FY end) | `ValuationReplayService::snapshot` (same date, AS_PER_MASTER) | per item: qty equal and value equal, **or** the qty difference equals the sum of challan_only / deferred-inward lines Books counted twice (listed with voucher numbers) → warning "explained", **or** (matching qty, differing unit cost only) every mismatched line is a stock-transfer receiving layer Books valued at zero cost (listed with voucher numbers) → warning "explained", **or** the difference is inside what the item's job-work receipts can account for — Books costs those from the challan value agreed with the job worker, Inventory refuses a commercial rate as a cost and reads the valuation posted on the line — listed per voucher with the expected and the maximum delta → warning "explained" |
